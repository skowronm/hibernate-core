...
import org.hibernate.envers.mappergenerator.CollectionMapperResolver;
...
public class AuditConfiguration {
	...
	public AuditConfiguration(Configuration cfg, ClassLoaderService classLoaderService) {
		...
		entCfg = new EntitiesConfigurator().configure(
				cfg, reflectionManager, globalCfg, auditEntCfg, auditStrategy, initializeCollectionMapperResolver(),
    ...
    private CollectionMapperResolver initializeCollectionMapperResolver() {
        ...
        try {
            Class<?> collectionMapperGeneratorClass =
                    Thread.currentThread().getContextClassLoader().loadClass(
                            auditEntCfg.getCollectionMapperResolverName());
            collectionMapperResolver = (CollectionMapperResolver) collectionMapperGeneratorClass.newInstance();
        } catch (Exception e) {
            throw new MappingException(String.format("Unable to create CollectionMapperResolver[%s] instance.",
                    auditEntCfg.getCollectionMapperResolverName()));
        }
